...
export default function Card({ item }: Props) {
  ...
  const createdDate = dayjs(item?.created_date).format("DD/MM/YYYY")
  const expiryDate = dayjs(item?.expiry_date).format("DD/MM/YYYY")
    ...
    <>
      <div className='p-5 rounded-[20px] border-2'>
        <div className="flex items-center justify-between">
          ...
          <p className="text-gray-500">{createdDate} - {expiryDate}</p>
        </div>
        <div>
      ...
      <DetailsModal item={item} open={open} setOpen={setOpen} />
    </>
  )
}